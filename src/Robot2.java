...
//		
		...
		System.out.println("Min no. of steps is : "+ min) ;
	...
	public static int removeObstacle(int numRows, int numColumns, List<List<Integer>> lot)
    {
        ...
        while(queue!=null) {
        	...
        	if(x+1>=0 && x+1 < numRows && y >=0 && y < numColumns && (grid[x+1][y] == 1 || grid[x+1][y]==9) && visited[x+1][y]==false ) {
        		visited[x+1][y]= true ;
        	...
        	if(x>=0 && x < numRows && y+1 >=0 && y+1 < numColumns && ( grid[x][y+1] == 1 || grid[x][y+1]==9) && visited[x][y+1]==false ) {
        		visited[x][y+1]= true ;
